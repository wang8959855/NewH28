...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.alertButton.hidden = NO;
            cell.alertWidth.constant = 85;
            cell.alertRight.constant = 20;
            if ([[[HCHCommonManager getInstance] UserTel] length] != 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.detailLabel.text = self.uploadInfoDic[@"systolicP"];
        }else if (indexPath.row == 8){
            cell.titleLabel.text = @"基准低压值";
            cell.detailLabel.text = self.uploadInfoDic[@"diastolicP"];
        ...
        if (indexPath.row == 0) {
            cell.phoneTextField.text = self.uploadInfoDic[@"friendTel1"];
            self.rafTel1TF = cell.phoneTextField;
        }else if (indexPath.row == 1){
            cell.phoneTextField.text = self.uploadInfoDic[@"friendTel2"];
            self.rafTel2TF = cell.phoneTextField;
        }else{
            cell.phoneTextField.text = self.uploadInfoDic[@"friendTel3"];
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    if (textField == _rafTel1TF) {
        [self.uploadInfoDic setObject:textField.text forKey:@"friendTel1"];
    }else if (textField == _rafTel2TF){
        [self.uploadInfoDic setObject:textField.text forKey:@"friendTel2"];
    }else if (textField == _rafTel3TF){
        [self.uploadInfoDic setObject:textField.text forKey:@"friendTel3"];
    }else{
        //血糖
    ...
    dispatch_sync(concurrentDiapatchQueue, ^
                   {
                       ...
                       dispatch_async(dispatch_get_main_queue(), ^{
                           [self uploadImageUrl:UPLOADHEADER parameter:@{@"userid":USERID} filePath:_imageFilePath];
                       });
...
- (void)requestAlertInfoUrl:(NSString *)url parameter:(NSDictionary *)parameter filePath:(NSString *)filePath{
    NSString *uploadUrl = [NSString stringWithFormat:@"%@/?token=%@",url,TOKEN];
    ...
    [[AFAppDotNetAPIClient1 sharedClient] globalmultiPartUploadWithUrl:uploadUrl fileUrl:filePath params:parameter Block:^(id responseObject, NSError *error) {
...
- (void)dateSureClick{
    ...
    if (_PickerViewType == PickerViewBirthday){
        ...
        [self.uploadInfoDic setObject:[NSString stringWithFormat:@"%ld",row+50] forKey:@"systolicP"];
        [self reloadTableViewSection:2 row:7];
    }else if (_PickerViewType == PickerViewDiastolicPressure){
        [self.uploadInfoDic setObject:[NSString stringWithFormat:@"%ld",row+30] forKey:@"diastolicP"];
...
- (void)uploadImageUrl:(NSString *)url parameter:(NSDictionary *)parameter filePath:(NSString *)filePath{
    [self addActityIndicatorInView:self.view labelText:NSLocalizedString(@"正在修改", nil) detailLabel:NSLocalizedString(@"正在修改", nil)];
    [self performSelector:@selector(loginTimeOut) withObject:nil afterDelay:60.f];
    ...
    NSString *uploadUrl = [NSString stringWithFormat:@"%@/?token=%@",url,TOKEN];
    [manager POST:uploadUrl parameters:parameter constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([[returnDic objectForKey:@"code"] integerValue] == 0) {
            ...
            [self.view makeToast:returnDic[@"message"] duration:1.5f position:CSToastPositionCenter];
        }
...
- (NSMutableDictionary *)uploadInfoDic {
    if (!_uploadInfoDic) {
        ...
        [_uploadInfoDic setObject:[hchc UserSystolicP] forKey:@"systolicP"];
        [_uploadInfoDic setObject:[hchc UserIsCHD] forKey:@"is_CHD"];
        [_uploadInfoDic setObject:[hchc UserDiastolicP] forKey:@"diastolicP"];
