...
- (IBAction)completeBtnAction:(id)sender
{
    ...
    if (min && max)
    {
        if (max < 40 || max > 180)
        {
            [self addActityTextInView:self.view text:NSLocalizedString(@"输入的心率必须在40到180之间", nil) deleyTime:1.5];
            return;
        }
        ...
        }else
        {
            if (max - min < 30)
            {
                [self addActityTextInView:self.view text:NSLocalizedString(@"最大心率值必须大于最小心率值30", nil) deleyTime:1.5];
                return;
            }
            else
            {
