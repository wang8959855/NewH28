...
- (void)setValueLabelOnChart:(NSMutableArray *)valueArray{
    ...
    if ([subObject isKindOfClass:[NSString class]]) {
        for (NSInteger i = 0; i < self.barArray.count; i++) {
            ...
            CGRect rect = [self.genericAxis.xLineValueArray[i] stringWidthRectWithSize:CGSizeMake(_barWidth + _barPadding * 0.5, 30) font:[UIFont systemFontOfSize:8]];
            
            if ([self.genericAxis.xLineValueArray[i] integerValue] != 0 )
            {
                ...
                popoverLabel.pattern = kPopoverLabelPatternBlank;
