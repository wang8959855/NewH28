...
#import "AlarmTableViewCell1.h"
#import "SMTabbedSplitViewController.h"
#import "EditAlarmViewController.h"
#import "AlarmCell.h"
#define kALARMVIEWCELL @"alarmViewCell"

@interface AlarmViewController1 ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation AlarmViewController1
...
//    self.alarmArray = @[NSLocalizedString(@"运动", nil),NSLocalizedString(@"约会",nil),NSLocalizedString (@"喝水", nil),NSLocalizedString (@"吃药",nil),NSLocalizedString(@"睡眠",nil)];
    ...
    [[PZBlueToothManager sharedInstance] getAlarmWithAlarmID:alarmID andAlarmModelBlock:^(AlarmModel *alarmModel) {
        if (alarmModel.state != 2 && alarmModel.repeats != 0)
        {
            if ([self.alarmIDArray[alarmModel.idNum] isEqualToString:@"0"]) {
                ...
                [weakSelf.alarmTableVIew reloadData];
            }
        }
...
- (void)setXibLabel
{
    ...
    [self.alarmTableVIew registerClass:[AlarmCell class] forCellReuseIdentifier:kALARMVIEWCELL];
}
...
- (IBAction)zdyAddAction:(UIButton*)button{
    ...
    if (model.idNum >= 0)
    {
        ...
        [self.view makeToast:@"闹钟最多可设置8个" duration:2.f position:CSToastPositionCenter];
    }
