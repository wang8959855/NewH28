...
@interface TrajectoryViewController ()<CAAnimationDelegate,MKMapViewDelegate,CLLocationManagerDelegate>
...
- (void)loadUI
{
    [self addNavWithTitle:NSLocalizedString(@"轨迹",nil) backButton:YES shareButton:YES];
    [self gpsStateLabel];
    [self distanceLabel];
    [self timeLabel];
    [self speedLabel];
    ...
    historyButton.sd_layout.topSpaceToView(self.view, 560 * kX)
...
- (void)mapModeClick
{
    ...
    {
        [self addActityTextInView:self.view text:NSLocalizedString(@"还未开始运动",nil) deleyTime:2.f];
        return;
    }
    if (!_backView)
    {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    CGPoint touchePoint = [touch locationInView:self.view];
...
- (UILabel *)gpsStateLabel
{
    if (!_gpsStateLabel)
    {
        ...
        imageView.frame = CGRectMake(25 * kX, 87 * kX, 25 * kX, 28 * kX);
        ...
        [self.view addSubview:_gpsStateLabel];
        ...
        mapButton.sd_layout.rightSpaceToView(self.view, 20 * kX)
        .centerYIs(imageView.centerY)
...
- (UILabel *)distanceLabel
{
    if (!_distanceLabel)
    {
        ...
        _distanceLabel.sd_layout.topSpaceToView(self.view, 77 * kX + 64)
...
- (UILabel *)timeLabel
{
    if (!_timeLabel)
    {
        ...
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        imageView.frame = CGRectMake(90 * kX, 226 * kX, 43 * kX, 25 * kX);
        [self.view addSubview:imageView];
...
- (UILabel *)speedLabel
{
    if(!_speedLabel)
    {
        ...
        [self.view addSubview:imageView];
        imageView.frame = CGRectMake(ScreenW - 90 *  kX - 43 * kX, 226 * kX, 43 * kX, 25 * kX);
...
- (UIButton *)beginBtn
{
    if (!_beginBtn)
    {
        ...
        button.sd_layout.topSpaceToView(self.view, 365 * kX)
        .centerXIs(self.view.centerX)
        .widthIs(100 * kX)
        .heightIs(100 * kX);
        _beginBtn = button;
