...
- (IBAction)addTimeAction:(UIButton *)sender {
    ...
    if (time >= 60) {
        [self.view makeToast:@"设置助眠时间最多60min" duration:1.5 position:CSToastPositionCenter];
    }else{
        time+=10;
...
- (IBAction)lessTimeAction:(UIButton *)sender {
    NSInteger time = self.timeLabel.text.integerValue;
    if (time <= 10) {
        [self.view makeToast:@"设置助眠时间最少10min" duration:1.5 position:CSToastPositionCenter];
    }else{
        time-=10;
        self.timeLabel.text = [NSString stringWithFormat:@"%ldmin",time];
    }
...
- (IBAction)localMusicAction:(UIButton *)sender {
    __weak HelpSleepViewController *weakSelf = self;
