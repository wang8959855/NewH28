...
@property (weak, nonatomic) UIImageView *SMSImageView;

@property (strong,nonatomic) UISwitch *stateSwitch;

@property (strong, nonatomic) UIView *backView;

@property (strong, nonatomic) NotifyModel *model;
...
- (void)viewDidLoad {
    ...
    [[PZBlueToothManager sharedInstance] getNotifyWithBlock:^(NotifyModel *notifyModel) {
        ...
        weakSelf.stateSwitch.on = notifyModel.SMSState;
        [weakSelf judgeShow];
    }];
...
- (void)loadUI
{
    [self addNavWithTitle:NSLocalizedString(@"短信提醒",nil) backButton:YES shareButton:NO];
    [self SMSImageView];
    ...
    tipLabel.sd_layout.topSpaceToView(_SMSImageView, 135 * kX)
    .centerXIs(ScreenW/2.)
    .widthIs(ScreenW)
    .heightIs(40);
    ...
    tipLabel1.text = NSLocalizedString(@"开启手机短信提醒",nil);
    ...
    .rightSpaceToView(self.stateSwitch, 3)
    .heightIs(_backView.height/2.);
    
    UILabel *tipLabel2 = [[UILabel alloc] init];
    tipLabel2.text = NSLocalizedString(@"开启后,手机来短信时,手环会震动提醒",nil);
...
{
    self.model.SMSState = self.stateSwitch.isOn;
    [self judgeShow];
...
- (void)judgeShow
{
    ...
    {
        self.SMSImageView.image = [UIImage imageNamed:@"SMSOpen"];
        self.backView.backgroundColor = kmainBackgroundColor;
    }
    else
    {
        self.SMSImageView.image = [UIImage imageNamed:@"SMSOff"];
...
- (UIImageView *)SMSImageView
{
    if (!_SMSImageView)
    {
        ...
        imageView.image = [UIImage imageNamed:@"SMSOpen"];
        ...
        .widthIs(70 * kX)
        .heightIs(70 * kX);
        _SMSImageView = imageView;
    }
    return _SMSImageView;
}
