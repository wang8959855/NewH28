...
- (instancetype)initWithTitles:(NSArray *)titles WithVCs:(NSArray *)childVCs WithColorArrays:(NSArray *)colors {
    if (self = [super init]) {
        ...
        self.frame = CGRectMake(0, 64, FUll_VIEW_WIDTH, FUll_CONTENT_HEIGHT - TabbarHeight);
        myArray = titles;
        classArray = childVCs;
        colorArray = colors;
...
//        self.selectVC = (TSOrderStatusBaseViewController *)NSClassFromString(classArray[0]);
//        pagerView.selectBlock = ^(NSInteger index){
//            self.selectVC = (TSOrderStatusBaseViewController *)NSClassFromString(classArray[index]);
//        };
        ...
        adaLog(@"You should correct titlesArray or childVCs count!");
    }
}
            ...
            if (page == i && i <= classArray.count - 1) {
                if ([classArray[i] isKindOfClass:[NSString class]]) {
                    ...
                    if (class && viewAlloc[i] == NO) {
                        ...
                        adaLog(@"您所提供的vc%li类并没有找到。  Your Vc%li is not found in this project!",(long)i + 1,(long)i + 1);
                    }
                }else {
                ...
                adaLog(@"您没有配置对应Title%li的VC",(long)i + 1);
            }else {
                /**<  内存管理限制控制器最大数量为5个   **/
...
- (void)createFirstViewController:(UIViewController *)ctrl {
    ...
    pagerView.scrollView.scrollEnabled =NO;
    /**<  新添加测试cache   **/
...
- (void)createOtherViewControllers:(UIViewController *)ctrl WithControllerTag:(NSInteger)i {
    ...
    if ([self.delegate performSelector:@selector(deallocVCsIfUnnecessary)]) {
        if (vcsArray.count > 5 && [self.delegate deallocVCsIfUnnecessary] == YES) {
            ...
            viewAlloc[deallocTag] = NO;
            if (isDebug) {
                adaLog(@"控制器%li被清除了",(long)deallocTag + 1);
            }            
            [vcsTagArray removeObjectAtIndex:0];
        }
    }
